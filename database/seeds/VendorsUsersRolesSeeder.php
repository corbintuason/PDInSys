<?php

use Illuminate\Database\Seeder;
use App\Traits\SeedsTrait;

class VendorsUsersRolesSeeder extends Seeder
{
    use SeedsTrait;
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        // Define Roles
        $roles = [
            (object) array(
                "name" => "Creator",
                "ability" => null
            ),
            (object) array(
                "name" => "Approver",
                "ability" => "approve"
            ),
        ];
        $generic_abilities = ["view-all", "view", "create"];

        // Will primarily be used in the email (role_text will be combined with the role title i.e 'vendorcreator@pd.com')
        $role_text = "vendor";

        // Create Roles
        $bouncer_roles = $this->createRoles($roles, $generic_abilities, "App\Vendor", $role_text);

        // Create Users
        $this->createUsers($bouncer_roles, "Vendor");
    }
}
