<template>
  <b-modal size="xl" id="open-department" hide-footer>
    <template v-slot:modal-title>{{department.name}}</template>
    <div class="d-block container">
      <h1 class="component-title">Current Clients</h1>
      <div class="row">
        <div class="col-md-12">
          <basic-table :items="department.clients" :fields="fields" :key="clients_key"></basic-table>
        </div>
      </div>
      <hr />
      <!-- CLIENT NAME -->
      <h1 class="component-title">Add Client</h1>
      <div class="row">
        <div class="col-md-2">
...
export default {
  ...
  methods: {
    addClient() {
      this.department.clients.push(this.new_client);
      this.new_client =  {
        salutation: null,
        last_name: null,
        first_name: null,
